...
// Icon map for certain kink tags (add more as desired)
const tagIcons = {
  'pegging': '🍑',
  'futanari': '🍆',
  'chastity_cage': '🔒',
...
function renderTagButtons(tags) {
  ...
  tags.forEach(tag => {
    const btn = document.createElement("button");
    btn.className = "tag-button";
    const icon = tagIcons[tag] ? `${tagIcons[tag]} ` : '';
    btn.textContent = `${icon}${tag.replaceAll('_', ' ')}`;
    btn.dataset.tag = tag;
...
// Load artists.json from file instead of embedded block
let allArtists = [];
