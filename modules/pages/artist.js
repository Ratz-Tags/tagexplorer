import { getArtistBySlug, preloadArtistBySlug, fetchArtistImages, } from '../api.js';
...
export async function initArtistPage() {
    ...
    if (!slug) {
        ...
        if (root)
            root.innerHTML = '<p>Missing artist identifier.</p>';
        return {};
    }
    ...
    if (!artist) {
        const root = document.querySelector('[data-artist-root]');
        if (root)
            root.innerHTML = '<p>Artist not found.</p>';
        return {};
    }
