import { initSidebar, setAllArtists as setSidebarArtists, } from '../sidebar.js';
import { initAudio, initAudioUI, syncAudioPanelLayout, toggleGlobalMute, getGlobalMuteState, triggerBassPulse, } from '../audio.js';
import { initTags, setAllArtists as setTagsArtists, setRenderArtistsCallback, setRandomBackgroundCallback, setTagTooltips, setTagTaunts, setTaunts, getActiveTags, getArtistNameFilter, renderTagButtons, setTagSearchMode, hydrateTagState, handleArtistNameFilter, } from '../tags.js';
import { initGallery, filterArtists, setRandomBackground, setAllArtists as setGalleryArtists, setGetActiveTagsCallback, setGetArtistNameFilterCallback, setSortMode, setSortPreference, getPaginationInfo, renderArtistsPage, getFilteredArtists, } from '../gallery.js';
import { initUI, setupInfiniteScroll, setupBackgroundRotation, showToast, vibrate, vibratePattern, } from '../ui.js';
import { loadAppData, persistGalleryState, restoreGalleryState, } from '../api.js';
...
import { initTagExplorer, toggleTagExplorer, setAllArtists as setExplorerArtists, } from '../tag-explorer.js';
...
import { evaluateRitualTriggers, getRitualCatalog, getStateSnapshot as getRitualStateSnapshot, registerCompletion as registerRitualCompletion, registerDismissal as registerRitualDismissal, registerReset as registerRitualReset, } from '../rituals/gallery-rituals.js';
import { initShameDossier, openShameDossier, getDossierEntries, } from '../shame-dossier.js';
import { incrementPressure } from '../progression/pressure-meter.js';
import { setIndulgenceLevel } from '../audio/humiliation-audio.js';
import { recordVisit as recordStreakVisit, getStreakState as getStreakSnapshot, getStreakTier as getCurrentStreakTier, getStreakTierInfo, isStreakTrackingEnabled, setStreakTrackingEnabled, onStreakChange as onStreakUpdate, } from '../progression/streaks.js';
...
async function loadCommandMotionModule() {
    if (shouldReduceMotionForCommands())
        return null;
...
async function setupCommandDeck() {
    ...
    const handleKeydown = (event) => {
        ...
        if (tagName) {
            ...
            if (['input', 'textarea', 'select'].includes(normalized))
                return;
        }
...
function applyMotionPreference(mode) {
    ...
    try {
        document.dispatchEvent(new CustomEvent('motion:change', { detail: { mode: normalized } }));
    }
    catch {
...
function setupPressureProgression() {
    ...
    const trackDepth = () => {
        ...
        if (!info)
            return;
        const marker = computePageMarker(info);
        if (!Number.isFinite(marker) || marker <= deepestPageSeen)
            return;
...
function openRitualOverlay(activation) {
    if (!activation || !activation.ritual)
        return;
...
function processRitualQueue() {
    if (currentRitualElement)
        return;
    if (!ritualQueue.length)
        return;
...
function queueRitualActivations(activations = []) {
    activations.forEach((activation) => {
        if (!activation?.id)
            return;
        if (currentRitualActivation?.id === activation.id)
            return;
...
function setupSidebarToggle() {
    ...
    copiedSidebarEl.addEventListener('click', (event) => {
        const target = event.target;
        if (target?.closest('.copied-sidebar-close')) {
    ...
    copiedSidebarEl.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            setSidebarHidden(true, { userInitiated: true });
        }
    });
    ...
    copiedSidebarEl.addEventListener('touchstart', (event) => {
        if (event.touches.length === 1) {
            touchStartX = event.touches[0].clientX;
        }
    });
    copiedSidebarEl.addEventListener('touchend', (event) => {
...
function setupMuteToggle() {
    ...
    document.addEventListener('audio:mutechange', (event) => {
        ...
        if (typeof detail?.muted === 'undefined')
            return;
        updateButtonState(Boolean(detail.muted));
    });
}
async function setupDossierButton() {
    ...
    document.addEventListener('dossier:toggle', (event) => {
        const detail = event.detail;
        setExpanded(Boolean(detail?.open));
    });
...
function setupMotionToggle(initialMode) {
    ...
    const updateButtonState = (mode) => {
        ...
        motionBtn.setAttribute('aria-label', isReduced ? 'Enable motion animations' : 'Reduce motion animations');
...
function createStreakWhisperLine(detail, artists = []) {
    if (!detail || !detail.trackingEnabled || !detail.didIncrement)
        return null;
...
function setupStreakBadges({ initialDetail } = {}) {
    ...
    const applyDetail = (detail) => {
        ...
        if (innerChip) {
            ...
            innerChip.setAttribute('aria-label', trackingEnabled
            ...
            if (innerValue)
                innerValue.textContent = trackingEnabled ? String(count) : '—';
            if (innerTier)
                innerTier.textContent = trackingEnabled ? tierLabel.toUpperCase() : 'GHOST MODE';
            if (innerLabel)
                innerLabel.textContent = count === 1 ? 'DAY CAPTURED' : 'DAY STREAK';
        }
        if (coverChip) {
            coverChip.setAttribute('aria-pressed', trackingEnabled ? 'true' : 'false');
            coverChip.setAttribute('aria-label', trackingEnabled
            ...
            if (coverValue)
                coverValue.textContent = trackingEnabled ? String(count) : '—';
        }
    };
    ...
    if (innerChip)
        innerChip.addEventListener('click', handleClick);
    if (coverChip)
        coverChip.addEventListener('click', handleClick);
    ...
    return () => {
        if (innerChip)
            innerChip.removeEventListener('click', handleClick);
        if (coverChip)
            coverChip.removeEventListener('click', handleClick);
...
function setupStreakSystem({ artists }) {
    ...
    return () => {
        if (typeof badgeCleanup === 'function')
            badgeCleanup();
        if (typeof unsubscribe === 'function')
            unsubscribe();
    };
...
function syncSidebarForFold(mode) {
    ...
    if (!sidebar || typeof sidebar._setSidebarHidden !== 'function')
        return;
...
function setupFoldModeSync({ foldAdapter, closeSettings, }) {
    ...
    const currentMode = (foldAdapter && typeof foldAdapter.getMode === 'function' && foldAdapter.getMode()) ||
...
function setupSortControls() {
    ...
    if (sortPreferenceElem) {
        sortPreferenceElem.addEventListener('change', (event) => {
            const target = event.target;
            if (target) {
                setSortPreference(target.value);
            }
        });
    }
}
async function setupFiltersButton() {
    ...
    filterButtons.forEach((btn) => {
        if (btn.dataset.tagExplorerToggleBound === 'true')
            return;
...
export async function initGalleryPage({ foldAdapter }) {
    ...
    if (quotes.length > 0) {
        ...
        if (taglineElem)
            taglineElem.textContent = String(random);
    }
    let hydrated = false;
    ...
    if (typeof savedState?.artistNameFilter === 'string' && savedState.artistNameFilter) {
        ...
        if (nameInput)
            nameInput.value = savedState.artistNameFilter;
        hydrated = true;
    }
    if (!hydrated) {
        renderTagButtons();
    ...
    return {
        ...
        onDispose: () => {
            persistState();
            if (typeof foldCleanup === 'function')
                foldCleanup();
            if (typeof idleCleanup === 'function')
                idleCleanup();
