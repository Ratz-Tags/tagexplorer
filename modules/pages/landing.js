...
import { getPressureState, onPressureChange, resetPressure, } from '../progression/pressure-meter.js';
...
const PRESSURE_CAPTIONS = [
    ...
    (levelText) => `Needle pricks intensify at ${levelText}%. Every filter you tap pushes the glow harder. Reset if you dare.`,
    (levelText) => `Archive crackle at ${levelText}%. The log is savouring every indulgence. Reset only delays the bite.`,
    (levelText) => `Critical overload at ${levelText}%. Whispers stop pretending to be gentle. Reset while you can still breathe.`,
];
...
function parseMissionProfile(raw) {
    ...
    try {
        ...
        if (!mission || !Number.isFinite(timestamp))
            return null;
...
function setupPressureMeter() {
    ...
    const applyState = (payload) => {
        const { description, numeric, text, tier, label } = describePressureState(payload?.level, payload?.tier);
    ...
    const unsubscribe = onPressureChange((detail) => {
...
function trapFocusWithin(container) {
    ...
    const handleKeyDown = (event) => {
        if (event.key !== 'Tab')
            return;
        const focusable = Array.from(container.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter((element) => !element.hasAttribute('disabled') &&
...
function setupMissionRitual() {
    ...
    function focusCurrentStep() {
        ...
        if (!currentStage)
            return;
        const target = currentStage.querySelector('button:not([disabled]), [href]:not([tabindex="-1"])');
        if (target) {
    ...
    function unlockCTA(profile) {
        ...
        if (profile) {
            try {
                document.dispatchEvent(new CustomEvent('landing:mission-set', {
                    detail: profile,
                }));
            }
            catch (error) {
