...
type DiscoverOptions = {
  maxPages?: number;
  perPage?: number;
  maxArtistsPerCombo?: number;
};

type KinkTagArtistManifest = Record<string, Array<{ artistName?: string; totalPosts?: number }>>;

type DanbooruPost = {
  tag_string_artist?: string;
};

type DanbooruCountsResponse = {
  counts?: {
    posts?: number;
  };
};

function hasIntegerPostCount(artist: ArtistRecord): artist is ArtistRecord & { postCount: number } {
  return typeof artist.postCount === 'number' && Number.isInteger(artist.postCount);
}

function meetsMinimumPosts(artist: ArtistRecord, min: number): artist is ArtistRecord & { postCount: number } {
  return hasIntegerPostCount(artist) && artist.postCount >= min;
}
...
const coreTags: readonly string[] = [
  'chastity_cage',
  'femdom',
  'futanari',
  'trap'
...
const extraTags: readonly string[] = [
  'bdsm',
  'hypnosis',
  'leash',
  'pet_play',
...
async function rateLimitedFetch(url: string, opts: RequestInit = {}, retries = 3): Promise<Response> {
...
async function tagExistsOnDanbooru(tag: string): Promise<boolean> {
  try {
    ...
    console.warn(`⚠️ could not verify ${tag} on Danbooru: ${getErrorMessage(err)}`);
    return true;
  }
}

async function getPostCountForQuery(query: string): Promise<number> {
  ...
  const data = await resp.json() as DanbooruCountsResponse;
  return data?.counts?.posts ?? 0;
}

async function getArtistTotalCount(artistName: string, cache: Map<string, number>): Promise<number> {
...
async function buildTagArtistCounts(tags: string[], artists: ArtistRecord[]): Promise<Record<string, ArtistCounts[]>> {
  const totalCache = new Map<string, number>();
  const result: Record<string, ArtistCounts[]> = {};
  for (const tag of tags) {
    const withTag = artists.filter(a => Array.isArray(a.kinkTags) && a.kinkTags.includes(tag));
    const rows: ArtistCounts[] = [];
    ...
    async function next(): Promise<void> {
      ...
      try {
        ...
        console.warn(`⚠️ count failed for ${current.artistName} ${tag}: ${getErrorMessage(e)}`);
      }
      return next();
    }
...
async function fetchPostsForQuery(query: string, page = 1, limit = 100): Promise<DanbooruPost[]> {
  ...
  return await resp.json() as DanbooruPost[];
}

async function discoverArtistsByCoreAndKink(core: readonly string[], kinkTags: string[], options: DiscoverOptions = {}): Promise<Map<string, Set<string>>> {
  const { maxPages = 100, perPage = 200, maxArtistsPerCombo = 0 } = options;
  const found = new Map<string, Set<string>>();
  for (const coreTag of core) {
    for (const tag of kinkTags) {
      const q = `${normalizeTag(coreTag)} ${normalizeTag(tag)}`;
      const collectedForCombo = new Set<string>();
      for (let p = 1; p <= maxPages; p++) {
        let posts: DanbooruPost[] = [];
        try {
          posts = await fetchPostsForQuery(q, p, perPage);
        } catch (e) {
          console.warn(`⚠️ discover failed for ${q} p${p}: ${getErrorMessage(e)}`);
          break;
        }
...
function mergeDiscoveredIntoArtists(existing: ArtistRecord[], discoveredMap: Map<string, Set<string>>): { updated: ArtistRecord[]; added: ArtistRecord[] } {
  const byName = new Map(existing.map((a): [string, ArtistRecord] => [a.artistName, a]));
  const added: ArtistRecord[] = [];
  for (const [artistName, tagSet] of discoveredMap.entries()) {
    ...
    if (entry) {
      ...
      const newArtist: ArtistRecord = { artistName: normName, kinkTags: addTags.sort() };
...
async function consolidateCountsFromKinkTagArtists(artists: ArtistRecord[]): Promise<{ artists: ArtistRecord[]; changed: boolean }> {
  let data: KinkTagArtistManifest | undefined;
  try {
    const raw = await readFile('kink-tag-artists.json', 'utf8');
    data = JSON.parse(raw);
  } catch {
  ...
  const byNameMax = new Map<string, number>();
...
async function updateKinkTags(): Promise<void> {
  let artists: ArtistRecord[];
  try {
    artists = JSON.parse(await readFile('artists.json', 'utf8'));
  } catch (err) {
  ...
  await writeFile('kink-tags.json', JSON.stringify(categorizedTags, null, 2) + '\n');
  ...
  if (added.length) {
    ...
    async function next(): Promise<void> {
      ...
      try {
        ...
        console.warn(`⚠️ postCount failed for ${a.artistName}: ${getErrorMessage(e)}`);
      }
      return next();
    }
    await Promise.all(Array.from({ length: limit }, () => next()));
  ...
  const removedArtists = finalArtists.filter(a => !meetsMinimumPosts(a, MIN_ARTIST_POSTS));
  finalArtists = finalArtists.filter((a): a is ArtistRecord & { postCount: number } => meetsMinimumPosts(a, MIN_ARTIST_POSTS));
  ...
  const changed = JSON.stringify(artists) !== JSON.stringify(finalArtists);
  if (changed) {
    await writeFile('artists.json', JSON.stringify(finalArtists, null, 2) + '\n');
  ...
  if (REFRESH_ZERO_COUNTS) {
    const zeroList = finalArtists.filter(a => !hasIntegerPostCount(a) || a.postCount <= 0);
    if (zeroList.length) {
      ...
      async function next(): Promise<void> {
        ...
        try {
          ...
          console.warn(`⚠️ zero-count refresh failed for ${a.artistName}: ${getErrorMessage(e)}`);
        }
        return next();
      }
      await Promise.all(Array.from({ length: limit }, () => next()));
  ...
  if (process.env.REFRESH_ALL_COUNTS === '1') {
    ...
    async function next(): Promise<void> {
      ...
      try {
        ...
        console.warn(`⚠️ refresh count failed for ${a.artistName}: ${getErrorMessage(e)}`);
      }
      return next();
    }
    await Promise.all(Array.from({ length: limit }, () => next()));
