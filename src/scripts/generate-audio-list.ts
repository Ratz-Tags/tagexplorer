...
interface AsmrManifest {
  generatedAt: string;
  totalLayers: number;
  layers: AsmrLayerEntry[];
  notes: string;
}
...
function toKebabCase(value: string | number | undefined | null): string {
  return String(value ?? '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'layer';
...
function assertAudioDirectory(): void {
...
function generateAudioFileList(): void {
  try {
    ensureDataDirectory();
    ...
    const audioData: AudioFileManifest = {
      generatedAt: new Date().toISOString(),
      totalFiles: mp3Files.length,
      files: mp3Files.map((filename): AudioFileEntry => ({
        filename,
...
function generateASMRManifest(): void {
  try {
    const layers: AsmrLayerEntry[] = [];
