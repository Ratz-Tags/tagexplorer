...
function openRitualOverlay(activation) {
  ...
  const element = document.createElement('te-gallery-ritual') as HTMLElement & {
    setFoldMode?: (mode: string) => void;
    configure?: (options: { ritual: unknown; foldMode: string }) => void;
  };
...
function setupVoiceSelectorButton(): void {
  const audioControls = document.querySelector('.audio-controls');
  if (!audioControls) return;

  let voiceBtn = document.getElementById('azure-voice-style-btn') as HTMLButtonElement | null;
  if (!voiceBtn) {
    voiceBtn = document.createElement('button') as HTMLButtonElement;
  ...
  if (!voiceBtn.dataset.selectorBound) {
    const updateExpandedState = (event: Event) => {
...
async function setupFavoritesButton(): Promise<void> {
  await customElements.whenDefined('te-command-bar');
  await new Promise(resolve => setTimeout(resolve, 0));

  const favoritesBtn = document.getElementById('favorites-btn') as HTMLButtonElement | null;
  const favoritesCount = document.getElementById('favorites-count') as HTMLElement | null;
  ...
  document.addEventListener('favorites:changed', (event: Event) => {
...
function setupSidebarToggle(): void {
  const copiedSidebarEl = document.getElementById('copied-sidebar') as SidebarElement | null;
  ...
  copiedSidebarEl.addEventListener('keydown', (event: KeyboardEvent) => {
    if (event.key === 'Escape') {
      setSidebarHidden(true, { userInitiated: true });
    }
  });
  ...
  copiedSidebarEl.addEventListener('touchstart', (event: TouchEvent) => {
    if (event.touches.length === 1) {
      touchStartX = event.touches[0].clientX;
    }
  });
  copiedSidebarEl.addEventListener('touchend', (event: TouchEvent) => {
  ...
  copiedSidebarEl._setSidebarHidden = (hidden, options: SidebarHideOptions = {}) => {
...
function setupAudioToggle(): void {
  ...
  const audioToggleButtons = Array.from(
    document.querySelectorAll<HTMLElement>('.audio-toggle'),
  );
...
function setupMuteToggle(): void {
  const muteBtn = document.getElementById('cover-mute-btn') as HTMLButtonElement | null;
  if (!muteBtn) return;
  ...
  const updateButtonState = (muted: boolean) => {
    ...
    const label = muteBtn.querySelector<HTMLElement>('.cover-command-label');
    if (label) {
  ...
  document.addEventListener('audio:mutechange', (event: Event) => {
    const detail = (event as CustomEvent<{ muted?: boolean }>).detail;
    if (typeof detail?.muted === 'undefined') return;
...
async function setupDossierButton() {
  ...
  ].filter((entry): entry is { el: HTMLElement; source: string } => entry.el instanceof HTMLElement);
  ...
  const setHasEntries = (hasEntries: boolean) => {
    entries.forEach(({ el }) => {
      if (!el) return;
      if (hasEntries) {
  ...
  document.addEventListener('dossier:toggle', (event: Event) => {
    const detail = (event as CustomEvent<{ open?: boolean }>).detail;
...
function setupMotionToggle(initialMode) {
  ...
  document.addEventListener('motion:change', (event: Event) => {
...
function setupStreakBadges({ initialDetail }: { initialDetail?: any } = {}): () => void {
  const innerChip = document.getElementById('streak-chip') as HTMLElement | null;
  const coverChip = document.getElementById('cover-streak-btn') as HTMLElement | null;
  if (!innerChip && !coverChip) {
    return () => {};
  }

  const announcers = Array.from(
    document.querySelectorAll<HTMLElement>('[data-streak-announcer]'),
  );
  const innerValue = innerChip?.querySelector<HTMLElement>('[data-streak-count]');
  const innerTier = innerChip?.querySelector<HTMLElement>('[data-streak-tier]');
  const innerLabel = innerChip?.querySelector<HTMLElement>('[data-streak-label]');
  const coverValue = coverChip?.querySelector<HTMLElement>('[data-streak-count]');
...
function updateCommandStatusLabels(mode: string | null | undefined): void {
...
type SidebarHideOptions = {
  userInitiated?: boolean;
};

type SidebarElement = HTMLElement & {
  _setSidebarHidden?: (hidden: boolean, options?: SidebarHideOptions) => void;
};

function syncSidebarForFold(mode: string | null | undefined): void {
  const sidebar = document.getElementById('copied-sidebar') as SidebarElement | null;
...
function setupFoldModeSync({
  foldAdapter,
  closeSettings,
}: {
  foldAdapter?: FoldAdapter | null;
  closeSettings?: () => void;
}): () => void {
...
function setupSortControls(): void {
  ...
  const sortPreferenceElem = document.getElementById('sort-preference') as HTMLSelectElement | null;
  if (sortPreferenceElem) {
    sortPreferenceElem.addEventListener('change', (event: Event) => {
...
async function setupFiltersButton() {
  ...
  ].filter((btn): btn is HTMLElement => Boolean(btn));
  ...
  document.addEventListener('tagFilters:toggle', (event: Event) => {
    const detail = (event as CustomEvent<{ open?: boolean }>).detail;
    const isOpen = Boolean(detail?.open);
    filterButtons.forEach((btn) => {
...
export async function initGalleryPage({ foldAdapter }: PageInitContext): Promise<PageLifecycle | void> {
  ...
  const quotes = Object.values(tooltips || {}).filter(Boolean) as string[];
  if (quotes.length > 0) {
    const random = quotes[Math.floor(Math.random() * quotes.length)];
    const taglineElem = document.getElementById('tagline') as HTMLElement | null;
    if (taglineElem) taglineElem.textContent = String(random);
  }
  ...
  if (typeof savedState?.artistNameFilter === 'string' && savedState.artistNameFilter) {
    ...
    const nameInput = document.getElementById('artist-name-filter') as HTMLInputElement | null;
...
export const initPage: PageInitializer = initGalleryPage;
