...
interface ArtistPageLifecycle extends PageLifecycle {
  beforeNavigate?: () => void;
}

interface ArtistPreviewPost {
...
async function renderPreview(container: HTMLElement, artist: ArtistRecord): Promise<void> {
  container.innerHTML = '';
  ...
  try {
    const posts = (await fetchArtistImages(artist.artistName, [], { limit: 1, order: 'score' })) as
      | ArtistPreviewPost[]
      | null
      | undefined;
    const post = Array.isArray(posts)
      ? posts.find((p) => (p?.large_file_url || p?.file_url) != null)
      : null;
    if (post) {
...
function renderLinks(container: HTMLElement, artist: ArtistRecord): void {
  container.innerHTML = '';
  const list = document.createElement('ul');
...
function updateFavoriteButton(button: HTMLButtonElement | null, artistName: string): void {
  if (!button) return;
...
export async function initArtistPage(): Promise<ArtistPageLifecycle | void> {
  loadFavorites();
  const slug = resolveSlug();
  if (!slug) {
    const root = document.querySelector<HTMLElement>('[data-artist-root]');
    if (root) root.innerHTML = '<p>Missing artist identifier.</p>';
    return {};
  }

  await preloadArtistBySlug(slug);
  const artist = (await getArtistBySlug(slug)) as ArtistRecord | null | undefined;
  if (!artist) {
    const root = document.querySelector<HTMLElement>('[data-artist-root]');
  ...
  const nameEl = document.querySelector<HTMLElement>('[data-artist-name]');
  const taglineEl = document.querySelector<HTMLElement>('[data-artist-tagline]');
  const previewEl = document.querySelector<HTMLElement>('[data-artist-preview]');
  const tagsEl = document.querySelector<HTMLElement>('[data-artist-tags]');
  const linksEl = document.querySelector<HTMLElement>('[data-artist-links]');
  const favoriteBtn = document.querySelector<HTMLButtonElement>('[data-artist-favorite]');
...
export const initPage: PageInitializer = initArtistPage;
