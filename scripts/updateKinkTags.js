...
function hasIntegerPostCount(artist) {
    return typeof artist.postCount === 'number' && Number.isInteger(artist.postCount);
}
function meetsMinimumPosts(artist, min) {
            ...
            if (resp.status === 429 || resp.status === 503) {
                ...
                if (ra) {
                    if (/^\d+$/.test(ra)) {
                        ...
                        if (!Number.isNaN(ts))
                            wait = Math.max(wait, ts - Date.now());
                    }
                }
...
async function getPostCountForQuery(query) {
    ...
    if (!resp.ok)
        throw new Error(`counts failed ${resp.status}`);
    const data = await resp.json();
...
async function fetchPostsForQuery(query, page = 1, limit = 100) {
    ...
    const resp = await rateLimitedFetch(url);
    if (!resp.ok)
        throw new Error(`posts failed ${resp.status}`);
    return await resp.json();
}
async function discoverArtistsByCoreAndKink(core, kinkTags, options = {}) {
        ...
        for (const tag of kinkTags) {
            ...
            for (let p = 1; p <= maxPages; p++) {
                ...
                for (const post of posts) {
                    ...
                    if (maxArtistsPerCombo && collectedForCombo.size >= maxArtistsPerCombo)
                        break;
                }
                if (maxArtistsPerCombo && collectedForCombo.size >= maxArtistsPerCombo)
                    break;
...
async function updateKinkTags() {
    ...
    for (const artist of artists) {
        ...
        if (!tags.some(tag => coreTags.includes(tag)))
            continue;
    ...
    finalArtists = finalArtists.filter((a) => meetsMinimumPosts(a, MIN_ARTIST_POSTS));
    ...
    if (removedCount > 0) {
        ...
        if (removedArtists.length <= 50)
            console.log('Removed artists:', removedArtists.map(a => a.artistName).join(', '));
        else
            console.log('Removed sample:', removedArtists.slice(0, 50).map(a => a.artistName).join(', ') + ' …');
    }
...
//# sourceMappingURL=updateKinkTags.js.map